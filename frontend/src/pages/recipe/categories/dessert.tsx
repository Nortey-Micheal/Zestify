...
import { ArrowBigLeftDashIcon, ArrowBigRightDashIcon, IceCreamBowlIcon, LoaderPinwheel } from "lucide-react"
import { useEffect, useState } from "react"
import { useSelector } from "react-redux"
import { toast } from "sonner"

export default function DessertRecipes() {
    const dessertRecipes = useSelector((state:RootState) => state.categoriesRecipes)
    const { error, isLoading, getRecipeByCategory } = useGetRecipeByCategory()
    const [page,setPage] = useState<number>(1)

    useEffect(() => {
        const fetchRecipe = async () => {
            await getRecipeByCategory({category: 'dessert',page})
        }
        fetchRecipe()
    },[page])
    ...
    return (
        <section className="relative">
            <h2 className="mb-3 flex gap-3 text-2xl items-center py-2"><IceCreamBowlIcon className="w-8 h-8"/> Dessert</h2>
            ...
            <ul className="w-full flex justify-center m-2 mt-10 bg-(--white) mx-auto rounded-lg gap-30 ">
                ...
                <li>
                    <button className="flex items-center gap-2 text-xl hover:bg-(--zesty-orange) hover:text-(--white) font-bold cursor-pointer rounded-lg disabled:bg-(--light-gray) px-2 my-1 disabled:text-(--light-grey) " disabled={(dessertRecipes.length < 6) || isLoading} onClick={() => setPage(prev => Math.max((prev + 1), 1))}>Next
